...
Ported to the MCP SDK client with the foxxe-mcp migration. Two shape
changes, both mechanical: fastmcp's `.data` is `structuredContent` (via
the `call` helper), and a failing tool comes back as `isError` rather
than raising `ToolError`.
"""
...
async def test_ctx_is_not_part_of_any_input_schema(client):
    # The SDK injects Context; it must not show up as an argument the
    # client is expected to supply.
    for tool in (await client.list_tools()).tools:
...
async def test_echo_round_trips(client, text):
    data = await call(client, "echo", {"text": text})
...
async def test_echo_rejects_a_missing_argument(client):
    ...
    # The SDK reports this as isError rather than raising, which is the
    # one assertion the framework change altered.
...
async def test_search_results_carry_the_deep_research_fields(client):
    results = (await call(client, "search", {"query": "x"}))["results"]
...
async def test_fetch_returns_the_marker(client, doc_id, marker):
    data = await call(client, "fetch", {"id": doc_id})
...
async def test_every_search_result_is_fetchable(client):
    ...
    results = (await call(client, "search", {"query": "x"}))["results"]
    for result in results:
        data = await call(client, "fetch", {"id": result["id"]})
...
async def test_fetch_of_an_unknown_id_degrades_quietly(client):
    ...
    assert result.structuredContent["title"] == "not found"
...
async def test_readme_resource_is_listed_and_readable(client):
    ...
    contents = (await client.read_resource("testy://readme")).contents
...
def test_forwarded_for_drops_the_originating_hop(raw, expected):
    assert testy_common.redact_forwarded_for(raw) == expected
...
def test_forwarded_for_never_echoes_the_client_address():
    ...
    assert "2a02:8086:cad::1" not in testy_common.redact_forwarded_for(
